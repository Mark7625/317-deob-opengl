...
public final class Rasterizer3D extends Raster {
    ...
    public static Background[] getTextures() {
        return textures;
    }
    ...
    public static void drawGouraud(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2) {
        try {
            drawGouraudd(i, j, k, l, i1, j1, k1, l1, i2);
        } catch (Exception e) {

        }
    }

    public static void drawTextured(int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2, int k2, int l2, int i3, int j3, int k3, int l3, int i4, int j4, int k4) {
        try {
             drawTexturedd(i, j, k, l, i1, j1, k1, l1, i2, j2, k2, l2, i3, j3, k3, l3, i4, j4, k4);
        } catch (Exception e) {

        }
    }

    public static void drawGouraudd(int i, int j, int k, int l, int i1, int j1, int k1, int l1,
                                   int i2) {
        if (GpuPlugin.process() && !renderOnGpu) {
            return;
        }
        int j2 = 0;
        int k2 = 0;
        ...
        if (i <= j && i <= k) {
            ...
            if (j < k) {
                ...
                if (i != j && j3 < j2 || i == j && j3 > l2) {
                    k -= j;
                    j -= i;
                    for (i = anIntArray1472[i]; --j >= 0; i += Raster.width) {
            ...
            if (i != k && j3 < j2 || i == k && l2 > j2) {
                j -= k;
                k -= i;
                for (i = anIntArray1472[i]; --k >= 0; i += Raster.width) {
        ...
        if (j <= k) {
            ...
            if (k < i) {
                ...
                if (j != k && j2 < l2 || j == k && j2 > j3) {
                    i -= k;
                    k -= j;
                    for (j = anIntArray1472[j]; --k >= 0; j += Raster.width) {
            ...
            if (j2 < l2) {
                k -= i;
                i -= j;
                for (j = anIntArray1472[j]; --i >= 0; j += Raster.width) {
        ...
        if (i < j) {
            ...
            if (l2 < j3) {
                j -= i;
                i -= k;
                for (k = anIntArray1472[k]; --i >= 0; k += Raster.width) {
        ...
        if (l2 < j3) {
            i -= j;
            j -= k;
            for (k = anIntArray1472[k]; --j >= 0; k += Raster.width) {
    ...
    private static void method375(int ai[], int i, int l, int i1, int j1, int k1) {
        ...
        if (world) {
            ...
            if (alpha == 0) {
                ...
                while (--k >= 0) {
                   ...
                   // ai[i++] = j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00);
                    //ai[i++] = j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00);
                    drawAlpha(ai, i++, j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), alpha);
                    drawAlpha(ai, i++, j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), alpha);
                    drawAlpha(ai, i++, j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), alpha);
                    drawAlpha(ai, i++, j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), j + ((ai[i] & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * j2 >> 8 & 0xff00), alpha);
                }
        ...
        do {
            ...
            drawAlpha(ai, i++, j + ((ai[i] & 0xff00ff) * k2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * k2 >> 8 & 0xff00), j + ((ai[i] & 0xff00ff) * k2 >> 8 & 0xff00ff) + ((ai[i] & 0xff00) * k2 >> 8 & 0xff00), alpha);
        } while (--k > 0);
    }

    public static void drawFlat(int i, int j, int k, int l, int i1, int j1, int k1) {
        if (GpuPlugin.process() && !renderOnGpu) {
            return;
        }
        int l1 = 0;
    ...
    public static void drawTexturedd(int i, int j, int k, int l, int i1, int j1, int k1, int l1,
