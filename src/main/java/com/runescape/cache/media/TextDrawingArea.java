...
public final class TextDrawingArea extends Raster {
    ...
    private void method392(byte abyte0[], int i, int j, int k, int l, int i1) {
        ...
        if (j + l >= Raster.bottomY)
            l -= ((j + l) - Raster.bottomY) + 1;
        if (i < Raster.topX) {
        ...
        if (i + k >= Raster.bottomX) {
            int l2 = ((i + k) - Raster.bottomX) + 1;
            k -= l2;
            l1 += l2;
            k1 += l2;
        }
        if (!(k <= 0 || l <= 0)) {
            method393(Raster.pixels, abyte0, i1, i2, j1, k, l, k1, l1);
        }
    }
    ...
    private void method394(int i, int j, byte abyte0[], int k, int l, int i1,
                           int j1) {
        int k1 = j + l * Raster.width;
        int l1 = Raster.width - k;
        int i2 = 0;
        int j2 = 0;
        if (l < Raster.topY) {
            int k2 = Raster.topY - l;
            i1 -= k2;
            l = Raster.topY;
            j2 += k2 * k;
            k1 += k2 * Raster.width;
        }
        if (l + i1 >= Raster.bottomY)
            i1 -= ((l + i1) - Raster.bottomY) + 1;
        if (j < Raster.topX) {
            int l2 = Raster.topX - j;
            k -= l2;
            j = Raster.topX;
            j2 += l2;
            k1 += l2;
        ...
        if (j + k >= Raster.bottomX) {
            int i3 = ((j + k) - Raster.bottomX) + 1;
            k -= i3;
            i2 += i3;
            l1 += i3;
        }
        if (k <= 0 || i1 <= 0)
            return;
        method395(abyte0, i1, k1, Raster.pixels, j2, k, i2, l1, j1, i);
    }
