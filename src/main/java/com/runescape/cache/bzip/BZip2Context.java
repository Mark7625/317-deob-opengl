package com.runescape.cache.bzip;// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 


public final class BZip2Context {

    public static int anIntArray587[];
    public final int[] anIntArray583;
    public final int[] anIntArray585;
    public final boolean[] aBooleanArray589;
    public final boolean[] aBooleanArray590;
    public final byte[] aByteArray591;
    public final byte[] aByteArray592;
    public final int[] anIntArray593;
    public final byte[] aByteArray594;
    public final byte[] aByteArray595;
    public final byte[][] aByteArrayArray596;
    public final int[][] anIntArrayArray597;
    public final int[][] anIntArrayArray598;
    public final int[][] anIntArrayArray599;
    public final int[] anIntArray600;
    public byte aByteArray563[];
    public int anInt564;
    public int anInt565;
    public int anInt566;
    public int anInt567;
    public byte aByteArray568[];
    public int anInt569;
    public int anInt570;
    public int anInt571;
    public int anInt572;
    public byte aByte573;
    public int anInt574;
    public boolean aBoolean575;
    public int anInt576;
    public int anInt577;
    public int anInt578;
    public int anInt579;
    public int anInt580;
    public int anInt581;
    public int anInt582;
    public int anInt584;
    public int anInt588;
    public int anInt601;

    public BZip2Context() {
        anIntArray583 = new int[256];
        anIntArray585 = new int[257];
        aBooleanArray589 = new boolean[256];
        aBooleanArray590 = new boolean[16];
        aByteArray591 = new byte[256];
        aByteArray592 = new byte[4096];
        anIntArray593 = new int[16];
        aByteArray594 = new byte[18002];
        aByteArray595 = new byte[18002];
        aByteArrayArray596 = new byte[6][258];
        anIntArrayArray597 = new int[6][258];
        anIntArrayArray598 = new int[6][258];
        anIntArrayArray599 = new int[6][258];
        anIntArray600 = new int[6];
    }
}
